...
export function Leaderboard({ onSelectEntry }: LeaderboardProps) {
  ...
  const getPodiumCardClass = (position: number, isMe: boolean) => {
    ...
    if (position < 3) {
      const meAccent = isMe ? ' ring-1 ring-[#f4bf4f]/18' : '';
      return `${podiumClasses[position]}${meAccent}`;
    }

    return isMe
      ? 'border border-[#f4bf4f]/25 shadow-[0_0_0_1px_rgba(244,191,79,0.08),0_0_18px_rgba(244,191,79,0.10)]'
      : '';
  };

  const getPodiumRankClass = (position: number) => {
    switch (position) {
      case 0:
        return 'bg-gradient-to-b from-[#f6c85f] to-[#d89a1f] text-[#0f0d0e] shadow-[0_8px_18px_rgba(244,191,79,0.24)]';
      case 1:
        return 'bg-gradient-to-b from-[#e5e7eb] to-[#aeb6c2] text-[#0f0d0e] shadow-[0_8px_18px_rgba(208,213,221,0.18)]';
      case 2:
        return 'bg-gradient-to-b from-[#d7996a] to-[#a95e27] text-white shadow-[0_8px_18px_rgba(201,122,61,0.18)]';
      default:
        return 'text-[#b8b2b3]';
    }
  };
  ...
  useLayoutEffect(() => {
    const nextRowIds = new Set(sorted.map((entry) => entry.id));

    for (const [rowId, previousTop] of previousRowTops.current.entries()) {
      if (!nextRowIds.has(rowId)) {
        previousRowTops.current.delete(rowId);
        continue;
      }
      ...
      const nextTop = rowNode.getBoundingClientRect().top;
    ...
    for (const entry of sorted) {
      ...
      if (!rowNode) continue;
      previousRowTops.current.set(entry.id, rowNode.getBoundingClientRect().top);
    }
  }, [sorted]);
          ...
          <div className="space-y-3">
            {sorted.map((entry, index) => {
              ...
              return (
                ...
                >
                  <Card
                    onClick={() => onSelectEntry?.(entry, isMe)}
                    hoverable={Boolean(onSelectEntry)}
                    className={getPodiumCardClass(index, isMe)}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 ${getPodiumRankClass(index)}`}
                      >
                        {index + 1}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <p className="text-white font-semibold truncate min-w-0" style={{ marginBottom: 0 }}>
                            {entry.name}
                          </p>
                        </div>
