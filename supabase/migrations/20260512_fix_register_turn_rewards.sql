-- Fix regression introduced in 20260325_make_geolocation_optional.sql and
-- 20260402_turn_registration_time_window.sql: reward calculation was replaced
-- with hardcoded values {xp:10, reputation:5, cachet:2} and the profiles table
-- was no longer updated. This migration restores the correct implementation:
--   • reads base_rewards and focus_role from the events table
--   • applies the focus_role role bonus (+15 XP / +7 cachet / +5 reputation)
--   • applies the token boost multiplier on the computed base
--   • updates profiles (XP, level, reputation, cachet, token_atcl)
--   • writes spend_boost and earn_turn entries to token_ledger
-- All features added in subsequent migrations are preserved:
--   • optional geolocation / geofence (20260325)
--   • 24-hour registration time-window (20260402)
--   • extended 10-column return type (20260402)
