-- Issue #324: server-side rule – one turn per event per user.
-- Adds a unique constraint on (user_id, event_id) to prevent duplicate registrations.
-- Updates the RPC to check for existing (user_id, event_id) pair before inserting,
-- returning an explicit 'already_registered' error.

-- 1. Unique partial index (allows event_id NULL for edge cases)
create unique index if not exists turns_user_event_unique_idx
  on public.turns (user_id, event_id)
  where event_id is not null;

-- 2. Replace the RPC to include the duplicate-event guard early in the flow
...
begin
  ...
  -- Idempotency: same client_action_id already processed
  select *
  ...
  -- #324: block duplicate event registration for the same user
  select *
    into v_existing_turn
  from public.turns
...
exception
  when unique_violation then
    -- Handle race condition: concurrent request for same (user_id, event_id)
    -- Check if it was an idempotent retry (same client_action_id)
    select *
      into v_existing_turn
    from public.turns
    where id = p_client_action_id
    ...
    -- Different client_action_id but same event → already registered
    raise exception 'already_registered';
end;
$$;
